...
<div class="box box-info">
            ...
            <!-- /.box-header -->
            <!-- form start -->
            <form class="form-horizontal" method="post" action="<?=$this->webroot?>users/edit/<?=$usuario['User']['id']?>">
              <div class="box-body">
                <div class="form-group">
                  <label for="inputEmail3" class="col-sm-2 control-label">Nome e Sobrenome</label>
                  <div class="col-sm-10">
                    <input type="text" name="data[User][name]" value="<?=$usuario['User']['name'];?>" class="form-control" id="inputEmail3" placeholder="Nome e Sobrenome">
                  </div>
							</div>
							<div class="form-group">
								<label for="inputEmail3" class="col-sm-2 control-label">Tipo de usuário</label>
								<div class="col-sm-10">
									<?php echo $this->Form->input('User.user_type_id', array(
									'type'=>'select',
									'label'=>false,
									'options'=> array($tipos),
									'default'=>$usuario['User']['user_type_id'],
									'class' => 'form-control'
									)); ?>
								</div>
							</div>
